	...
	$('#count').click(function() {
		...
		for (const string_i in comments) {
			...
			if (string[0] == '#') {
				...
				is_doz = false;
			} else if (is_doz && /(Камышовые заросли|Травянистый берег|Редколесье|Разрушенная ограда|Расколотое дерево|Лесной ручеёк|Валежник|Дубрава|Илистая тропа|Нагретые камни|Одинокий склон|Главный туннель|Активный \d)/u.test(string)) {
				let s = string.split(':');
				if (s.length != 2) {
